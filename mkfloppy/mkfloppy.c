...
static uint16_t
fat_find_next_free(uint16_t clus) {
    do {
        clus++;
    } while (fat[clus] != 0);
    return clus;
...
static uint8_t*
get_data_for_clus(uint16_t clus) {
    return data + 512 * (14 + (clus - 2));
}

static int
write_filename(char buffer[11], char* fn) {
    char* f = fn;
    int fncount = 0;
    while (!(*f == '.' || *f == 0)) {
        fncount++;
...
static int
write_file_from_disk_to_floppy(fat_node* fnode, char* filename) {
...
static void
write_fat_entries(dirinfo* parent, char* fname, char* sname, int dirflag) {
    printf("Processing: %s\n", fname);
...
static void
write_root_entries(dirinfo* parent, char* fname, char* sname, int dirflag) {
    printf("Processing: %s\n", fname);
...
main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg[0] != '-') {
            if (out_fn == 0)
                ...
                fprintf(
                    stderr, "Output filename specified more than once:\n\t%s\n\t%s\n", out_fn, arg);
                exit(1);
            }
            ...
            if (boot_fn == 0)
                ...
                fprintf(stderr, "Boot image specified more than once:\n\t%s\n\t%s\n", boot_fn, arg);
                exit(1);
            }
            ...
            if (dir_fn == 0)
                ...
                fprintf(stderr, "Directory specified more than once:\n\t%s\n\t%s\n", dir_fn, arg);
                exit(1);
            }
        } else {
    ...
    // Write boot sector
    if (boot_fn != 0) {
        FILE* boot_file = fopen(boot_fn, "rb");
        if (boot_file == NULL) {
